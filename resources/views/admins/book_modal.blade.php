    ...
    <div id="bookingModal" class="modal">
        <div class="modal-content">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Quick Booking</h3>
                        ...
                        <div class="mt-2">
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div class="lg:col-span-2">
                                    ...
                                    <div class="mt-6">
                                        <h4 class="text-md font-medium text-gray-900 mb-3">Select Accommodation Type *</h4>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div id="accommodation-option" class="selection-card border rounded-lg p-4 cursor-pointer selected" onclick="selectAccommodationType('accommodation')">
                                                <div class="flex items-center">
                                                    <div class="mr-3">
                                                        <i class="fas fa-hotel text-2xl text-blue-500"></i>
                                                    </div>
                                                    <div>
                                                        <h5 class="font-medium text-gray-900">Accommodation</h5>
                                                        <p class="text-sm text-gray-500">Book one or multiple rooms in our main building</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div id="cottage-option" class="selection-card border rounded-lg p-4 cursor-pointer" onclick="selectAccommodationType('cottage')">
                                                <div class="flex items-center">
                                                    <div class="mr-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-sm text-gray-500">Book one of our standalone cottages (day use only)</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div id="accommodation-error" class="error-message hidden mt-2">Please select accommodation type</div>
                                    </div>

                                    <!-- Room Selection (shown when accommodation is selected) -->
                                    <div id="room-selection-section" class="mt-6">
                                    ...
                                    <!-- Cottage Selection (hidden by default) -->
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse lg:hidden">
                    <button type="button" id="mobile-create-booking-btn" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Create Booking
                    </button>
    ...
    <script>
        ...
        const roomTypes = {
            ...
            suite: [
                ...
                { id: 8, name: "Presidential Suite", description: "Our most luxurious accommodation", price: 10000, maxGuests: 4, type: "suite" }
            ]
        };
        ...
        // Track selected items and entrance fees
        let selectedRooms = [];
        let selectedCottages = [];
        let currentAccommodationType = 'accommodation'; // default
        let entranceFees = {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadRooms();
            loadCottages();
            selectAccommodationType('accommodation');
            setupPhoneInput();
            setupValidation();
            setupDatePickers();
            updateGuestSummary();
            ...
            document.getElementById('first-name').addEventListener('input', updateGuestSummary);
            document.getElementById('last-name').addEventListener('input', updateGuestSummary);
            document.getElementById('customer-email').addEventListener('input', updateGuestSummary);
            document.getElementById('customer-phone').addEventListener('input', updateGuestSummary);
            document.getElementById('check-in').addEventListener('change', updateDatesSummary);
            document.getElementById('check-out').addEventListener('change', updateDatesSummary);
        });

        // Update guest information in summary
        ...
        function updateDatesSummary() {
            ...
            if (checkin && checkout) {
                ...
                if (currentAccommodationType === 'cottage') {
        ...
        function setupValidation() {
            ...
            const mobileCreateBtn = document.getElementById('mobile-create-booking-btn');
            
            createBtn.addEventListener('click', function() {
                if (validateForm()) {
                    // Form is valid, proceed with booking creation
        ...
        function validateForm() {
            ...
            if (currentAccommodationType === 'accommodation' && !checkout.value) {
            ...
            if (currentAccommodationType === 'accommodation' && selectedRooms.length === 0) {
            ...
            if (currentAccommodationType === 'cottage' && selectedCottages.length === 0) {
                ...
                roomElement.innerHTML = `
                    ...
                    <div class="mt-4">
                        <label class="flex items-center">
                            <input type="checkbox" class="form-checkbox h-4 w-4 text-blue-600" onclick="event.stopPropagation()">
                            <span class="ml-2 text-sm text-gray-700">Include Breakfast</span>
                        </label>
                    </div>
                `;
        ...
        // Load cottages into the UI (without breakfast option)
        ...
        // Select accommodation type (rooms or cottages)
        function selectAccommodationType(type) {
            currentAccommodationType = type;
            
            // Update UI for accommodation type selection
            if (type === 'accommodation') {
                document.getElementById('accommodation-option').classList.add('selected');
                ...
                document.getElementById('accommodation-option').classList.remove('selected');
        ...
        // Toggle selection of a room or cottage
        function toggleSelection(element, type) {
            ...
            if (type === 'room') {
                ...
                if (element.classList.contains('selected')) {
                    // Add to selected cottages
                    selectedCottages.push({ id, title, price });
                } else {
                    // Remove from selected cottages
        ...
        function updateSelectedItemsSummary() {
            ...
            if (currentAccommodationType === 'accommodation') {
