	...
	<script>
			...
			categoryPieChart = new Chart(ctx, {
				...
				data: {
					...
					datasets: [{
						...
						borderColor: '#ffffff',
						borderWidth: 1, // Thinner border for compact look
						hoverOffset: 8  // Reduced hover effect
					}]
				},
				options: {
					responsive: true,
		...
		function updateStats(stats) {
			if (stats) {
				...
				const month = monthNames[parseInt($('#month').val()) - 1];
				const year = $('#year').val();
		...
		function updateComparisonValues(comparison) {
			...
			$('#earningsChangeValue').text(earningsChange + '%');
			if (earningsChange >= 0) {
			...
			$('#bookingsChangeValue').text(bookingsChange + '%');
			if (bookingsChange >= 0) {
